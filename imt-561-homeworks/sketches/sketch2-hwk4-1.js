...
registerSketch('sk2', function(p) {
    ...
    p.keyPressed = function() {
        if (showPauseMenu) {
            var focused = null;
            for (var i = 0; i < pauseInputs.length; i++)
                if (pauseInputs[i].focused) { focused = pauseInputs[i]; break; }
            if (focused) {
                if (p.keyCode === p.BACKSPACE) { focused.value = focused.value.slice(0, -1); return false; }
                if (p.key >= '0' && p.key <= '9' && focused.value.length < 5) { focused.value += p.key; } else if (p.key === '.' && focused.value.indexOf('.') === -1 && focused.value.length < 4) { focused.value += '.'; }
            }
            return false;
        }
        var focused = null;
        for (var i = 0; i < inputs.length; i++)
            if (inputs[i].focused) { focused = inputs[i]; break; }
        if (!focused) return;
        if (p.keyCode === p.BACKSPACE) { focused.value = focused.value.slice(0, -1); return false; }
        if (p.key >= '0' && p.key <= '9' && focused.value.length < 5) { focused.value += p.key; } else if (p.key === '.' && focused.value.indexOf('.') === -1 && focused.value.length < 4) { focused.value += '.'; }
        return false;
    ...
    function drawDoneScreen() {
        ...
        p.textStyle(p.BOLD);
        p.textAlign(p.CENTER, p.CENTER);
        p.text(cycleCount + (cycleCount === 1 ? ' shot made!' : ' shots made!'), W / 2, 460);
